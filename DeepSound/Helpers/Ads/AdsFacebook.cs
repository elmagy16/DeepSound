        ...
        public static AdView InitAdView(Activity activity, LinearLayout adContainer, RecyclerView recyclerView)
        {
            try
            {
                if (DeepSoundTools.GetStatusAds() && AppSettings.ShowFbBannerAds)
                {
                    ...
                    // Request an ad
                    adView.BuildLoadAdConfig().WithAdListener(new BannerAdListener(recyclerView)).Build();
                    adView.LoadAd();
...
#pragma warning restore 618
                            // Request an ad
                            //interstitialAd.BuildLoadAdConfig().WithAdListener(new MyInterstitialAdListener(activity, interstitialAd)).Build();
...
#pragma warning restore 618
                            //rewardVideoAd.BuildLoadAdConfig().WithAdListener(new MyRRewardVideoAdListener(activity, rewardVideoAd)).Build();
...
#pragma warning restore 618
                                        ...
                                        nativeAd.BuildLoadAdConfig().WithAdListener(new NativeAdListener(activity, nativeAd, nativeAdLayout)).Build();
                                        nativeAd.LoadAd();
                                        break;
                                    }
                                default:
    ...
    //{
    //    private readonly NewsFeedAdapter NativePostAdapter2;
    //    public MyNativeAdsManagerListener(NewsFeedAdapter nativePostAdapter)
    //    {
    //        try
    //        {
